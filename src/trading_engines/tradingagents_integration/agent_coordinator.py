"""
KHAZAD_DUM TradingAgents Coordinator
Complete orchestration of TradingAgents with KHAZAD_DUM context
"""
...
class AgentWrapper:
    def __init__(
        self, khazad_dum_database, use_ibkr=None, ibkr_port=None
    ):
        """
        Initialize KHAZAD_DUM TradingAgents wrapper with IBKR integration
        ...
        # Initialize KHAZAD_DUM components
        self.regime_detector = RegimeDetector()
        self.portfolio_provider = PortfolioContextProvider(
            khazad_dum_database, use_ibkr=self.use_ibkr, ibkr_port=self.ibkr_port
        )
        ...
        print(f"KHAZAD_DUM Setup: IBKR {ibkr_status}, Mode: {mode} (port {self.ibkr_port})")
    ...
    def analyze_stock(self, symbol, date=None, market_context=None):
        ...
        if portfolio_context.get("data_source") == "IBKR + KHAZAD_DUM":
    ...
    """
        ...
        # Enhance the result with KHAZAD_DUM context
        enhanced_result = {
            "symbol": symbol,
            "decision": decision,
            "raw_result": result,
            ...
            "khazad_dum_enhancement": enhanced_context,
        }
    ...
    def get_portfolio_summary(self):
        ...
        print(f"\n{'='*60}")
        print(
            f"KHAZAD_DUM PORTFOLIO SUMMARY - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        print(f"{'='*60}")
