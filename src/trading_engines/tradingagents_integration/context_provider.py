...
Handles portfolio data from both KHAZAD_DUM database and IBKR
"""
...
class PortfolioContextProvider:
    """Provides portfolio context from KHAZAD_DUM database or IBKR"""

    def __init__(self, khazad_dum_database, use_ibkr=False, ibkr_port=4002):
        """
        Initialize portfolio context provider
        
        Args:
            khazad_dum_database: KHAZAD_DUM database instance
            use_ibkr: Whether to use IBKR for live data
            ibkr_port: IBKR port (4001=live, 4002=paper)
        """
        self.db = khazad_dum_database
    ...
    def get_portfolio_context(self):
        ...
        try:
            khazad_dum_context = self._get_khazad_dum_context()
            
            if self.use_ibkr and self.ibkr_connector:
                ibkr_data = self._get_ibkr_context()
                if ibkr_data:
                    return self._merge_contexts(khazad_dum_context, ibkr_data)
            
            return khazad_dum_context
        except Exception as e:
            logger.error(f"Error getting portfolio context: {e}")
            return self._get_fallback_context()

    def _get_khazad_dum_context(self):
        """Get historical context from KHAZAD_DUM database"""
        try:
            recent_regime_query = """
            ...
            logger.error(f"Error getting KHAZAD_DUM context: {e}")
            return self._get_fallback_context()
    ...
    def _merge_contexts(self, khazad_dum_context, ibkr_context):
        """Merge KHAZAD_DUM and IBKR contexts"""
        merged = ibkr_context.copy()
        merged.update({
            "recent_regimes": khazad_dum_context["recent_regimes"],
            "recent_candidates": khazad_dum_context["recent_candidates"],
            "data_source": "IBKR + KHAZAD_DUM",
        })
        
        if merged["current_positions"] and khazad_dum_context["recent_regimes"]:
            current_regime = khazad_dum_context["recent_regimes"][0]["regime"] if khazad_dum_context["recent_regimes"] else "neutral"
